...
class StudentsDataImport implements ToModel, WithHeadingRow
{
    ...
    public function model(array $row)
    {
        ...
        if (!empty($row['nis'])) {
            ...
            // Buat wali jika data tidak kosong
            $guardianId = null;
            if (array_filter($guardianData)) {
                $guardian = new Guardian($guardianData);
                $guardian->save();
            ...
            // Buat atau perbarui entitas user yang terkait dengan siswa
            $userEmail = $row['email_ortu'] ?? $row['email_wali'];
            ...
            // Hubungkan user dengan siswa jika user belum memiliki student_id
            if (!$user->student_id) {
                $user->student_id = $student->id;
                $user->save();
            }

            // Buat entitas HeightWeight terkait dengan siswa
            $heightWeight = new HeightWeight([
                'student_id' => $student->id,
                'height' => $row['tinggi'] ?? null,
                'weight' => $row['berat'] ?? null,
