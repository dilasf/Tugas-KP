<x-app-layout>
    ...
    <div class="bg-white rounded-lg shadow-md mx-4 overflow-hidden">
        <div class="flex justify-end px-6 py-4">
            <x-primary-button tag="a" href="{{route('subject.create')}}">
                <span class="text-12px ml-1">{{ __('+ Tambah Mata Pelajaran') }}</span>
            </x-primary-button>
        </div>
        <div class="text-black max-h-[calc(100vh-200px)] overflow-y-auto">
            <x-table header="Header Content" :sidebarOpen="$sidebarOpen" class="overflow-x-auto mx-auto">
                ...
                <tr class="text-center">
                    ...
                    <td :class="{ 'flex flex-row justify-center items-center gap-2': sidebarOpen, 'justify-center items-center gap-2': !sidebarOpen }">
                        <x-edit-primary-button tag="a" href="{{ route('subject.edit', ['id' => $mapel->id]) }}"
                        ...
                        <x-danger-button
                            x-data=""
                            x-on:click.prevent="
                                $dispatch('open-modal', 'confirm-subject-deletion');
                                $dispatch('set-action', '{{ route('subject.destroy', $mapel->id) }}');"
                            class="flex items-center justify-center min-w-[60px]">
            ...
            <x-modal name="confirm-subject-deletion" focusable maxWidth="xl">
