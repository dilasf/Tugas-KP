<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-10">
        <div class="top-0 left-0 w-full h-10 rounded-t-md bg-light-blue flex items-center justify-center text-white font-semibold text-md leading-tight">
            {{ __('Formulir Tambah Data Semester dan Tahun Ajaran') }}
        </div>
        <div class="bg-white overflow-hidden shadow-sm">
            <div class="p-6 text-black dark:text-gray-100">
                <div class="max-h-[70vh] overflow-y-auto">
                    <form method="post" action="{{ route('subject.semester_year.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                        @csrf
                        <div class="max-w-3xl">
                            <x-input-label for="semester" value="Semester" />
                            <x-text-input id="semester" type="text" name="semester" class="mt-1 block w-full bg-zinc-100" value="{{ old('semester')}}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('semester')" />
                        </div>

                        <div class="max-w-3xl">
                            <x-input-label for="year" value="Tahun Terbit" />
                            <x-text-input id="year" type="number" name="year" class="mt-1 block w-full bg-zinc-100" value="{{ old('year')}}" required />
