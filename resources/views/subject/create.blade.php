<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-10">
        <div class="top-0 left-0 w-full h-10 rounded-t-md bg-light-blue flex items-center justify-center text-white font-semibold text-md leading-tight">
            {{ __('Formulir Tambah Data Mata Pelajaran') }}
        </div>
        <div class="bg-white overflow-hidden shadow-sm">
            <div class="p-6 text-black dark:text-gray-100">
                <div class="max-h-[70vh] overflow-y-auto">
                    <form method="post" action="{{ route('subject.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                        @csrf
                        <div class="max-w-3xl">
                            <x-input-label for="subject_name" value="Mata Pelajaran" />
                            <x-text-input id="subject_name" type="text" name="subject_name" class="mt-1 block w-full bg-zinc-100" value="{{ old('subject_name')}}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('subject_name')" />
                        </div>

                        <div class="max-w-3xl">
                            <x-input-label for="kkm" value="KKM" />
                            <x-text-input id="kkm" type="number" name="kkm" class="mt-1 block w-full bg-zinc-100" value="{{ old('kkm')}}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('kkm')" />
                        </div>

                        <div class="flex justify-end space-x-4 mt-4 w-full">
                            <x-secondary-button tag="a" href="{{ route('subject.index') }}">Batal</x-secondary-button>
