                ...
                <div class="max-h-[70vh] overflow-y-auto">
                    <form method="post" action="{{ route('student_data.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                        ...
                        <div class="max-w-3xl">
                            ...
                            <x-text-input id="nis" type="text" name="nis" class="mt-1 block w-full bg-zinc-100" value="{{ old('nis')}}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('nis')" />
                        </div>

                        <div class="max-w-3xl">
                            <x-input-label for="nisn" value="NISN" />
                            <x-text-input id="nisn" type="text" name="nisn" class="mt-1 block w-full bg-zinc-100" value="{{ old('nisn')}}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('nisn')" />
                        </div>

                        <div class="max-w-3xl">
                            <x-input-label for="nipd" value="NIPD" />
                            <x-text-input id="nipd" type="text" name="nipd" class="mt-1 block w-full bg-zinc-100" value="{{ old('nipd')}}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('nipd')" />
                        </div>

                        <div class="max-w-3xl">
                            <x-input-label for="student_name" value="Nama Siswa" />
                            <x-text-input id="student_name" type="text" name="student_name" class="mt-1 block w-full bg-zinc-100" value="{{ old('student_name')}}" required />
                        ...
                        <div class="max-w-3xl">
                            ...
                            <x-text-input id="nik" type="text" name="nik" class="mt-1 block w-full bg-zinc-100" value="{{ old('nik')}}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('nik')" />
                        </div>

                        <div class="max-w-3xl">
                            <x-input-label for="place_of_birth" value="Tempat Lahir" />
                            <x-text-input id="place_of_birth" type="text" name="place_of_birth" class="mt-1 block w-full bg-zinc-100" value="{{ old('place_of_birth')}}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('place_of_birth')" />
                        </div>

                        <div class="max-w-3xl">
                            <x-input-label for="date_of_birth" value="Tanggal Lahir" />
                            <x-text-input id="date_of_birth" type="date" name="date_of_birth" class="mt-1 block w-full bg-zinc-100" value="{{ old('date_of_birth')}}" required />
                        ...
                        <div class="max-w-3xl">
                         ...
                         <!-- Form input untuk data tinggi dan berat badan -->
                        <div class="max-w-3xl mb-4">
                            <x-input-label for="height" value="Tinggi Badan (cm)" />
                            <x-text-input id="height" type="number" name="height" class="mt-1 block w-full bg-zinc-100" value="{{ old('height') }}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('height')" />
                        </div>

                        <div class="max-w-3xl mb-4">
                            <x-input-label for="weight" value="Berat Badan (kg)" />
                            <x-text-input id="weight" type="number" name="weight" class="mt-1 block w-full bg-zinc-100" value="{{ old('weight') }}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('weight')" />
                        </div>

                        <div class="max-w-3xl mb-4">
                            <x-input-label for="head_size" value="Ukuran Kepala (cm)" />
                            <x-text-input id="head_size" type="number" name="head_size" class="mt-1 block w-full bg-zinc-100" value="{{ old('head_size') }}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('head_size')" />
                        </div>
                         <!-- End Form input untuk data tinggi dan berat badan -->

                         <!-- Form input untuk data siswa -->
                        <div class="max-w-3xl">
                            <x-input-label for="previous_school" value="Asal Sekolah" />
                            <x-text-input id="previous_school" type="text" name="previous_school" class="mt-1 block w-full bg-zinc-100" value="{{ old('previous_school')}}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('previous_school')" />
                        </div>

                        <div class="max-w-3xl">
                            <x-input-label for="birth_certificate_number" value="No Akta" />
                            <x-text-input id="birth_certificate_number" type="text" name="birth_certificate_number" class="mt-1 block w-full bg-zinc-100" value="{{ old('birth_certificate_number')}}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('birth_certificate_number')" />
                        </div>

                        <div class="max-w-3xl">
                            <x-input-label for="child_number" value="Anak Ke" />
                            <x-text-input id="child_number" type="number" name="child_number" class="mt-1 block w-full bg-zinc-100" value="{{ old('child_number')}}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('child_number')" />
                        </div>

                        <div class="max-w-3xl">
                            <x-input-label for="number_of_siblings" value="Jumlah Saudara" />
                            <x-text-input id="number_of_siblings" type="number" name="number_of_siblings" class="mt-1 block w-full bg-zinc-100" value="{{ old('number_of_siblings')}}" required />
                    ...
                    <div class="max-w-3xl">
                        ...
                        <x-select-input id="transportation" name="transportation" class="mt-1 block w-full bg-zinc-100" required placeholder="Tentukan Alat Transportasi">
                            ...
                            <option value="Kendaraan Umum" {{ old('transportation') == 'kendaraan umum' ? 'selected' : '' }}>Kendaraan Umum</option>
                            <option value="Kendaraan Pribadi" {{ old('transportation') == 'kendaraan pribadi' ? 'selected' : '' }}>Kendaraan Pribadi</option>
                        </x-select-input>
                    ...
                    <div class="max-w-3xl">
                        ...
                        <x-text-input id="distance_to_school" type="number" name="distance_to_school" class="mt-1 block w-full bg-zinc-100" value="{{ old('distance_to_school')}}" required />
...
<script>
    document.getElementById('toggleParentForm').addEventListener('click', function() {
        window.location.href = '{{ route('student_data.create-parent') }}';
    });

    document.getElementById('toggleGuardianForm').addEventListener('click', function() {
        window.location.href = '{{ route('student_data.create-guardian') }}';
    });
</script>
