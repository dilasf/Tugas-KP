<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 py-10">
        <div class="top-0 left-0 w-full h-10 rounded-t-md bg-light-blue flex items-center justify-center text-white font-semibold text-md leading-tight">
            {{ __('Formulir Tambah Data Prestasi') }}
        </div>
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white">
                <form method="POST" action="{{ route('achievements.store', ['studentId' => $student->id, 'semester_year_id' => $semester_year_id]) }}">
                    @csrf

                    <div class="max-w-3xl">
                        <x-input-label for="achievement_type" value="Jenis Prestasi" />
                        <x-text-input id="achievement_type" type="text" name="achievement_type" class="mt-1 block w-full bg-zinc-100" value="{{ old('achievement_type')}}" required />
                    ...
                    <div class="max-w-3xl py-8">
                        ...
                        <x-text-input id="description" type="text" name="description" class="mt-1 block w-full bg-zinc-100" value="{{ old('description')}}" />
