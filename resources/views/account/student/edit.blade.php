        ...
        <div class="bg-white overflow-hidden shadow-sm">
            <div class="p-6 text-black">
                <div class="max-h-[70vh] overflow-y-auto">
                    <form method="post" action="{{ route('account.student.update', $studentAccount->id) }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="max-w-3xl">
                            ...
                            <x-text-input id="name" type="text" name="name" class="mt-1 block w-full bg-zinc-100" value="{{ old('name', $studentAccount->user->name) }}"/>
                            <x-input-error class="mt-2" :messages="$errors->get('name')" />
                        </div>

                        <div class="max-w-3xl">
                            <x-input-label for="email" value="Email"/>
                            <x-text-input id="email" type="email" name="email" class="mt-1 block w-full bg-zinc-100" value="{{ old('email', $studentAccount->user->email) }}"/>
