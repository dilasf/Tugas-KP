<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-10">
        <div class="top-0 left-0 w-full h-10 rounded-t-md bg-light-blue flex items-center justify-center text-white font-semibold text-md leading-tight">
            {{ __('Formulir Edit Data Assessment') }}
        </div>
        <div class="bg-white overflow-hidden shadow-sm">
            <div class="p-6 text-black">
                <div class="max-h-[70vh] overflow-y-auto">
                    <form method="post" action="{{ route('grade.skill_scores.update', $assessmentType->assessment_type) }}" class="mt-6 space-y-6">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="flex justify-end space-x-4 mt-4 w-full">
                            <x-secondary-button tag="a" href="{{ route('grade.skill_scores.index') }}">Batal</x-secondary-button>
