...
<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 py-10">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
            <form method="POST" action="{{ route('height_weights.update', ['studentId' => $student->id, 'heightWeightId' => $heightWeight->id, 'aspectName' => $aspectName, 'semester_year_id' => $semester_year_id]) }}">
                @csrf
                @method('PATCH')
                <input type="hidden" name="semester_year_id" value="{{ $semester_year_id }}">
